...
public class FileUtils {
	...
	public Path getRelativePath(File baseDir, File file) {
		...
		if (!filePathString.startsWith(baseDirPathString)) {
			throw new IllegalArgumentException(
					String.format("Expected file path \"%\" to start with directory path: \"%s\"", filePathString,
							baseDirPathString));
		}
	...
	public void assertIsFileWithSize(File expectedFile) {
		...
		if (expectedFile.length() == 0) {
			throw new FileAssertionException(
					String.format("Expected file to have size but was empty: %s", expectedFile));
		}
	}
