...
public class PspiIndexGeneratorCLI {
    ...
    private static void printUsage(Options options) {
        ...
        try(PrintWriter writer = new PrintWriter(buffer)) {
            ...
            writer.println("    java -jar pspi-index-generator.jar -i inputDir -o outputDir -r SMALL -c center-middle -q 0.9");
            writer.println();
        }
    ...
    private static Options makeOptions() {
        ...
        options.addOption(Option.builder("r")
                                    ...
                                    .hasArg()
                                    .argName("SMALL|LARGE")
                                    .desc("Automatically resizes images to standard PSPI size specifications.  Acceptable values are \"SMALL\" for 320x400px or \"LARGE\" for 640x800px.")
                                    .build());
        options.addOption(Option.builder("c")
                .longOpt("crop-anchor")
                .hasArg()
                .argName("(top|center|bottom)-(left|middle|right)")
                .desc("When -r is specified, this value is used to override the default crop anchor, which is: " + PspiIndexGenerator.DEFAULT_CROP_ANCHOR).build());
        options.addOption(Option.builder("q")
                                    .longOpt("quality")
                                    .hasArg()
                                    .argName("0.0-1.0")
                                    .desc("Compression quality level to use when images are manipulated (crop or resize).  Values range is 0.0-1.0, where 0.0 is least quality and 1.0 is highest quality.  Default value is 0.7")
                                    .build());
        return options;
    }
}
