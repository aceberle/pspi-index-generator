...
public class ResizeImageModifier implements ImageModifier {
	...
	public BufferedImage modifyImage(File imageFile, BufferedImage origImage) {
		...
		eventHandler.info("Resizing image \"%s\" from %sx%s to %sx%s", imageFile.getName(), origWidth, origHeight,
		...
		if (!almostEqual(origRatio, targetRatio, 0.001)) {
			eventHandler.error(
					"Warning: Original aspect ratio %s does not match target aspect ratio %s, image will be warped! Consider enabling auto-croping!",
					origRatio, targetRatio);
		}
