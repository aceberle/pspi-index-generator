...
public class PspiIndexGeneratorGUI extends JFrame {
	...
	public PspiIndexGeneratorGUI(Preferences preferences) {
		...
		inputDirBrowseButton.addActionListener(
				chooseDirectory(() -> findFirst(inputDirText, outputDirText, lastSelectedFile), path -> {
		...
		outputDirBrowseButton.addActionListener(
				chooseDirectory(() -> findFirst(inputDirText, dataFilePathText, lastSelectedFile), path -> {
					final String pathString = path.getAbsolutePath();
		...
		getContentPane()
				.setLayout(new MigLayout("", "[85px][312px,grow][69px]", "[][][][][][][][][][][][][][][][][][]"));
		...
		addRadioButtonGroup(PREF_CROP_ANCHOR, topLeftCropRadio, topMiddleCropRadio, topRightCropRadio,
				centerLeftCropRadio, centerMiddleCropRadio, centerRightCropRadio, bottomLeftCropRadio,
				bottomMiddleCropRadio, bottomRightCropRadio);
		...
		dataFileBrowseButton.addActionListener(
				chooseExcelFile(() -> findFirst(dataFilePathText, inputDirText, lastSelectedFile), path -> {
					final String pathString = path.getAbsolutePath();
		...
		generateButton.addActionListener(e -> {
			...
			final PspiIndexGenerator.Builder builder = new PspiIndexGenerator.Builder().verboseLogging(true, out, err)
					.strict(strictCheckBox.isSelected()).imageFolderPattern(imageFolderPatternText.getText())
			...
			if (overrideImageNamesCheckBox.isSelected()) {
				...
				if (StringUtils.isBlank(text)) {
					JOptionPane.showMessageDialog(this, "Expected Output Image Name Pattern to be provided!", "Error",
			...
			pool.execute(() -> {
				try {
					...
					final int rc = JOptionPane.showOptionDialog(this,
							"The output directory is not empty! Are you sure that you want to proceed?", "Warning",
					...
					ex.printStackTrace(err);
					final Object[] options = { "Add to existing PSPI Files", "Delete existing files and proceed",
							"Cancel" };
					final int rc = JOptionPane.showOptionDialog(this,
							"The output directory alreadt contains PSPI files! Please choose an option:", "Warning",
	...
	private void generateAndAppendOutput(final JTextPane consoleTextPane, final PrintStream err,
			final PspiIndexGenerator.Builder builder, final File inputDirectory, final File outputDirectory) {
		try {
			clearTextPane(consoleTextPane);
	...
	private void cleanOutputAndGenerate(final JTextPane consoleTextPane, final PrintStream err,
			final PspiIndexGenerator.Builder builder, final File inputDirectory, final File outputDirectory) {
		try {
			clearTextPane(consoleTextPane);
	...
	private void resetPreferences() {
		...
		compressionQualityModel.setValue(
				preferences.getFloat(PREF_COMPRESSION_QUALITY, PspiIndexGenerator.DEFAULT_COMPRESSION_QUALITY * 100));
		...
		imageFilePatternText
				.setText(preferences.get(PREF_IMAGE_FILE_PATTERN, PspiIndexGenerator.DEFAULT_IMAGE_FILE_PATTERN));
		imageFolderPatternText
				.setText(preferences.get(PREF_IMAGE_FOLDER_PATTERN, PspiIndexGenerator.DEFAULT_IMAGE_FOLDER_PATTERN));
	...
	private ActionListener chooseExcelFile(Supplier<File> startAtSupplier, Consumer<File> textConsumer) {
		return getFileChooserActionListener(startAtSupplier, textConsumer, JFileChooser.FILES_ONLY,
				new FileNameExtensionFilter("Excel Spreadsheet (*.xslx)", "xlsx"));
	}

	private ActionListener getFileChooserActionListener(Supplier<File> startAtSupplier, Consumer<File> textConsumer,
			final int fileSelectionMode, final FileFilter fileFilter) {
		return e -> {
