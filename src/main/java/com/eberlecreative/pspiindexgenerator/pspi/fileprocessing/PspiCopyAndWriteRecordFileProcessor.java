...
public class PspiCopyAndWriteRecordFileProcessor implements FileProcessor {
	...
	public PspiCopyAndWriteRecordFileProcessor(EventHandler eventHandler, FileUtils fileUtils, ImageCopier imageCopier,
			OutputFileNameResolver outputFileNameResolver, ImageUtils imageUtils, RecordWriter indexRecordWriter,
	...
	@Override
	public void processFile(File inputDirectory, File outputDirectory, String imageFolderName, File imageFile,
	...
	private String getImageSize(File imageFile) throws IOException {
		...
		if (fileSize > MAX_IMAGE_SIZE) {
			eventHandler.error(
					"Invalid image found! Expected size to be less than %s bytes but found size of %s bytes, image at: %s",
					MAX_IMAGE_SIZE, fileSize, imageFile);
		}
		...
		if (ratio != 0.8) {
			eventHandler.error(
					"Invalid image found! Expected aspect ratio of 0.8 but found %spx / %spx = %s, image at: %s", width,
					height, ratio, imageFile);
		}
		return "OTHER";
	}
