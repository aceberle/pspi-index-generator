...
public class PspiDirectoryValidator {
    ...
    public Set<String> validatePspiDirectory(File directory) {
        ...
        try {
            ...
            try (Reader in = new FileReader(indexFile)) {
                ...
                for(CSVRecord record : records) {
                    ...
                    if(!directoryName.equals(volume)) {
                        throw new InvalidPspiDirectoryException(String.format("Expected \"Volume\" to be \"%s\" but was \"%s\" in record #%s", directoryName, volume, record.getRecordNumber()));
                    }
            ...
            throw new InvalidPspiDirectoryException(String.format("Exception occurred while verifying directory!: %s", directory), e);
        }
    }
