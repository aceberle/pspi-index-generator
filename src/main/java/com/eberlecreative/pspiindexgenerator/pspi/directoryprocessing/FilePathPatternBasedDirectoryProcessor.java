...
public class FilePathPatternBasedDirectoryProcessor implements DirectoryProcessor {
	...
	public FilePathPatternBasedDirectoryProcessor(FileUtils fileUtils, EventHandler eventHandler,
			Pattern imageFolderPattern, Pattern imageFilePattern,
			FieldValueRepositoryFactory fieldValueRepositoryFactory) {
			...
			if (!imageFolder.isDirectory()) {
				...
				for (File imageFile : fileUtils.sort(imageFolder.listFiles())) {
					...
					if (!imageFile.isFile()) {
						...
						fileProcessor.processFile(inputDirectory, outputDirectory, imageFolder.getName(), imageFile,
								fieldValues);
					}
				}
			}
				...
				if (value != null) {
					...
					if (existingValue != null) {
						throw new RuntimeException(String.format(
								"Image path already contains value for key '%s', existing value='%s', new value='%s'",
